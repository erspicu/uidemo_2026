...
public class DataViewModel : ViewModelBase
{
    ...
    public DataViewModel() { foreach (var p in _source) FilteredPeople.Add(p); }
    ...
    private void FilterData()
    {
        ...
        foreach (var p in _source)
            if (string.IsNullOrEmpty(q) || p.Name.ToLower().Contains(q) ||
                p.Department.ToLower().Contains(q) || p.Status.ToLower().Contains(q))
